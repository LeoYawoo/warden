...
Position::Position(const NTempest::C3Vector &pos) : m_position(pos) {
}
...
NTempest::C3Vector Position::GetPosition() const {
    return m_position;
}

void Position::SetPosition(const NTempest::C3Vector &pos) {
    m_position = pos;
}
...
bool Position::IsFacing(const Position &target, float threshold) const {
    NTempest::C3Vector dir = DirectionTo(target);
    dir.Normalize();
...
float Position::DistanceTo(const NTempest::C3Vector &pos) const {
...
NTempest::C3Vector Position::DirectionTo(const Position &other) const {
...
Position Position::operator+(const NTempest::C3Vector &offset) const {
...
bool Position::IsFacing(const Position &source, const Position &target, const Position &reference, float *dotProduct) {
    NTempest::C3Vector dirToTarget = target.DirectionTo(source);
    NTempest::C3Vector dirToRef = reference.DirectionTo(source);
